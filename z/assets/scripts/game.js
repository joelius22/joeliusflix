...
//NOW REDUNDANT
//if (categoryArray[4] == "s") { //formerly "s"
//    isSchool == true;
//}
...
if (os == "Android" || os == "Android-Silk" || os == "iOS") {
    touchDevice = true;
}
...
backButton.addEventListener('click', function (e) {
    ...
    //////////////////////
    //trial-override - all desktop users see new games
    if (!touchDevice) {
        returnURL = "https://www.friv.com/new.html";
    }
    //////////////////////
    ...
    //if (isCoUk && categoryArray[4] == "a" && !touchDevice) {
    //    returnURL = "https://www.friv.co.uk/new.html";
    //}
...
gameEffectsButton.addEventListener("click", function () {
    ...
    switch (effectsCounter) {
        ...
        case 22:
            ...
            if (flashGame) { //full "roll" rotation is poor for Flash games
...
//analytics
if (isSchool) {
    gtag('config', 'UA-124684579-7', { //school (100% sample, all pages and events)
        'send_page_view': false,
        'sample_rate': 100
    });
} else {
    if (os == "Desktop") {
        gtag('config', 'UA-124684579-6', { //windows and other users (x% sample, no pages, events only) TYPICAL SAMPLE RATE: 10
            'send_page_view': false,
            'sample_rate': 10
        });
    } else {
        if (os == "iOS") { //ios users to own account
            gtag('config', 'UA-124684579-2'); //A remember, also on !fv
        } else {
            gtag('config', 'UA-124684579-6', { //SWITCH 4-6 all other users (x% sample, no pages, events only)
                'send_page_view': false,
                'sample_rate': 100
            });
        }
...
if (game.spare == 1 || externallyFramed) {
...
//special for Amazon-Silk browser
if (/silk/i.test(userAgent)) {
    categoryArray[0] = "Amazon-Silk";
}


//special for Chromebooks
//if (/\bCrOS\b/.test(userAgent)) {
//    categoryArray[0] = "Desktop-ChromeOS";
//}

//special for old Windows (earlier than Windows 10. Analytics works, but little difference in performance)
//if((navigator.appVersion.indexOf("Windows ") != -1) && (navigator.appVersion.indexOf("Windows NT 10.") == -1 )){ //is windows, but is not windows 10
//    categoryArray[0] = "Desktop-oldWindows";
//}
...
if (externallyFramed) {
    ...
    categoryArray[0] = "External-Framing";
}
...
//EXPERIMENTAL DISBALE RIGHT MOUSE BUTTON
/////////////////////////////////////////
